...
/mob/living/attacked_by(obj/item/I, mob/living/user)
	...
	if(I.force_dynamic)
		...
		// Affix-driven % enhanced damage scales base damage (melee)
		if(hascall(I, "get_affix_stat"))
			var/dmg_pct = call(I, "get_affix_stat")("damage_pct_bonus", 0)
			if(isnum(dmg_pct) && dmg_pct != 0)
				newforce = max(0, round(newforce * (1 + dmg_pct), 1))
		apply_damage(newforce, I.damtype, def_zone = hitlim)
		// Affix-driven extra elemental damage (e.g., Flaming: +fire damage)
		if(hascall(I, "get_affix_stat"))
			var/extra_fire = call(I, "get_affix_stat")("damage_fire_bonus", 0)
			if(isnum(extra_fire) && extra_fire > 0)
				apply_damage(round(extra_fire), BURN, def_zone = hitlim)
        		var/extra_tox = call(I, "get_affix_stat")("damage_toxin_bonus", 0)
        		if(isnum(extra_tox) && extra_tox > 0)
        			apply_damage(round(extra_tox), TOX, def_zone = hitlim)
        		var/extra_oxy = call(I, "get_affix_stat")("damage_oxy_bonus", 0)
        		if(isnum(extra_oxy) && extra_oxy > 0)
        			apply_damage(round(extra_oxy), OXY, def_zone = hitlim)
		if(I.damtype == BRUTE)
			next_attack_msg.Cut()
	...
	if(I.force_dynamic)
		// Reflect: defender returns some damage to attacker based on worn armor affixes
		if(isliving(src) && isliving(user))
			var/mob/living/defender = src
			var/mob/living/attacker = user
			if(hascall(defender, "get_total_affix_stat"))
				var/refl_flat = call(defender, "get_total_affix_stat")("reflect_flat", 0)
				if(isnum(refl_flat) && refl_flat > 0)
					attacker.apply_damage(round(refl_flat), BRUTE)
		return TRUE
...
/obj/item/proc/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(force_dynamic && !user.used_intent.tranged && !user.used_intent.tshield)
		if(proximity_flag && isopenturf(target) && !user.used_intent?.noaa)
			...
			if(istype(user.rmb_intent, /datum/rmb_intent/swift))
				adf = max(round(adf * CLICK_CD_MOD_SWIFT), CLICK_CD_INTENTCAP)
			// Affix: Increased Attack Speed on the weapon reduces click cooldown
			if(hascall(src, "get_affix_stat"))
				var/ias = call(src, "get_affix_stat")("attack_speed_pct", 0)
				if(isnum(ias) && ias > 0)
					ias = clamp(ias, 0, 0.8)
					adf = max(round(adf * (1 - ias)), CLICK_CD_INTENTCAP)
			user.changeNext_move(adf)
			playsound(get_turf(src), pick(swingsound), 100, FALSE, -1)
			user.aftermiss()
		if(!proximity_flag && ismob(target) && !user.used_intent?.noaa) //this block invokes miss cost clicking on seomone who isn't adjacent to you
			...
			// Affix: Increased Attack Speed on the weapon reduces click cooldown
			if(hascall(src, "get_affix_stat"))
				var/ias2 = call(src, "get_affix_stat")("attack_speed_pct", 0)
				if(isnum(ias2) && ias2 > 0)
					ias2 = clamp(ias2, 0, 0.8)
					adf = max(round(adf * (1 - ias2)), CLICK_CD_INTENTCAP)
			user.changeNext_move(adf)
			playsound(get_turf(src), pick(swingsound), 100, FALSE, -1)
			user.aftermiss()
